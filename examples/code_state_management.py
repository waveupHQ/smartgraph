...
class LanguageLearningApp(ReactiveSmartGraph):
    ...
    async def process_answer(self, correct: bool) -> Dict[str, Any]:
        result = await self.execute_and_await("LanguageLearning", {"action": "process_answer", "correct": correct})
        return result
...
async def main():
    ...
    for _ in range(10):  # Simulate 10 rounds
        ...
        print(f"Progress: {progress.get('correct', 0)}/{progress.get('total', 0)} " \
